...
export const CoinCounter = ({ style, animation }: any) => {
  ...
  const targetedScore = useTeaserStore(state => state.gameConfig.targetedScore);
  const state = useTeaserStore(state => state.state);
  const gameConfig = useTeaserStore(state => state.gameConfig);

  return (
    <MotionBox display="flex" alignItems="center" justifyContent="center" gap="8px" {...style} {...animation}>
      ...
      <Flex>
        <PixelatedNumber
          number={count}
          fontSize="28px"
          animation={count !== 0 && state === 'in-game' ? `${bounce} 1s ease-in-out` : ''}
        />
      </Flex>
    </MotionBox>
  );
